...
std::string knownSensationString(KnownSensation sensation) {
  switch (sensation) {
    ...
    case RandomWalk::Sensations::KnownSensation::
...
Ultraleap::Haptics::result<Ultraleap::Haptics::SensationPackage>
LibrarySensation::loadSensationPackage(std::string package) {
  const auto sensation_package_result =
      Ultraleap::Haptics::SensationPackage::loadFromFile(*lib, package);
...
int LibrarySensation::start(KnownSensation sensation) {
  ...
  if (!pattern) {
    std::cerr << "Failed to get " << sensation_name << " sensation"
              << std::endl;
    return 1;
  }
  ...
  if (sensation_name.rfind("Hand.", 0) == 0) {
    ...
    auto on_frame_callback = [&](const Leap::Controller& controller) {
      ...
      if (!hands.isEmpty()) {
        ...
        (*sensation_instance)
            .set("hand", LeapHandConverter::invalidElementSimpleHand());
      }
  ...
  if (!set_result || !emitter_result) {
    std::cerr << "Failed to set " << sensation_name
              << " sensation or to start the sensation emitter" << std::endl;
    return 2;
  }
  return 0;
}
      ...
      switch (current_sensation) {
        ...
        case RandomWalk::Sensations::KnownSensation::
          ...
          if (result.has_value()) {
            ...
            std::cout << std::endl
                      << "Update failed: is Radius a valid named parameter?"
                      << std::endl;
          }
          break;
        default:
