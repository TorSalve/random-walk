        ...
        void* user_pointer)
    {
        ...
        for (TimePointOnOutputInterval& sample : interval)
        {
            ...
            if (config->mode == RenderMode::STATIC) {
                ...
                offset = config->pattern.evaluate_at(t, config->offsets) + Vector3(-20.f, 0.f, 0.f);
            }
            Ultrahaptics::Vector3 position = leapOutput.palm_position + (offset * (leapOutput.hand_is_left ? -1 : 1));
...
#pragma endregion INIT_EMITTER
        ...
        while (true)
        {
            ...
            try
            {
                ...
                case Utils::hash("i"):
                    pattern_current = std::clamp(pattern_current - 1, 0, (int)available_patterns.size() - 1);
                ...
                std::cout << "int: " << point.intensity << " | freq: " << point.frequency << " | off: " << point.leap_offset << " | dur: " << point.pattern.get_duration() << " | action: " << action << std::endl;
            }
