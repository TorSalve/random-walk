...
public:
    ...
    OrientedBoundingBox(const Vector3& p_centre, const Vector3& p_axis1, const Vector3& p_axis2, const Vector3& p_axis3, float p_slen1, float p_slen2, float p_slen3)
        : centre(p_centre)
        , axis1(p_axis1.normalize())
        , axis2(p_axis2.normalize())
        , axis3(p_axis3.normalize())
        , extent1(p_slen1 / 2.0f)
        , extent2(p_slen2 / 2.0f)
        , extent3(p_slen3 / 2.0f)
    {
    }
    ...
    bool contains(float x, float y, float z) const
    {
        ...
        float dist1 = diff.dot(axis1);
        float dist2 = diff.dot(axis2);
        float dist3 = diff.dot(axis3);
